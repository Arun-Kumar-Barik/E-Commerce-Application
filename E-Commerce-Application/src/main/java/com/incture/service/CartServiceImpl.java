...
import com.incture.repository.CartItemDao;
import com.incture.repository.CustomerDao;
import com.incture.repository.ProductDao;
import com.incture.repository.SessionDao;
...
public class CartServiceImpl implements CartService {
	...
	public Cart addProductToCart(CartDTO cartDto, String token) {
		...
		//initialize if cart is null (check on saturday)
		if (customerCart == null) {
	        customerCart = new Cart();
	        customerCart.setCustomer(existingCustomer);
	        customerCart.setCartItems(new ArrayList<>());  // Initialize empty list
	        customerCart.setCartTotal(0.0);
	        
	        customerCart = cartDao.save(customerCart);
	    }
...
//		Integer cartId = existingCustomer.getCustomerCart().getCartId();
		...
		if (existingCustomer.getCustomerCart() == null) {
